...
The backend exposes \texttt{/api/bootstrap} to reveal local demo credentials. The frontend uses that only for convenience. Real authentication occurs through \texttt{/api/auth/login}, which issues a random token stored in SQLite. Protected routes are covered by \texttt{requireAuth}.
...
The frontend is a single client-rendered dashboard page. This is a good choice for a small internal tool because all controls and results are visible together. The user does not need to navigate across many pages to understand the system.

\section{State Management}
The frontend uses React state to track sessions, selected session ID, selected session details, simulation form fields, live form fields, replay file state, authentication state, and loading state. It then uses \texttt{useMemo} to derive table-ready arrays from the selected session.
...
\textbf{What it does.} It selects the input source based on mode, runs feature extraction, correlation, path ranking, estimation, evaluation, optional logging, and final report rendering. It returns a structured dictionary containing meta, summary, estimates, evaluation, paths, correlations, sampled raw events, and the terminal report.
...
\textbf{Why it matters.} It shows that the project was designed with deployment and reproducibility in mind, even though live capture remains a host-sensitive feature.
...
\textbf{Important pieces.} \texttt{PacketRecord}, \texttt{FlowBurst}, \texttt{load\_pcap\_events()}, \texttt{rows\_to\_events()}, \texttt{\_extract\_tshark\_rows()}, \texttt{\_rows\_to\_packets()}, \texttt{\_packets\_to\_events()}, \texttt{\_build\_flow\_bursts()}, \texttt{\_cluster\_bursts()}, \texttt{\_cluster\_to\_sequences()}, and helper functions for mapping, classification, flow keys, and protocol detection.

\textbf{Why it matters.} This file is the bridge from low-level captured packets to the higher-level path abstraction used by the core algorithm. It is also the most heuristic-heavy file in the repository.
...
\textbf{Dependencies.} \texttt{express}, \texttt{cors}, \texttt{multer}, and \texttt{ws}.
...
\textbf{Why it matters.} It isolates token extraction, login, and route protection from route definitions.
...
\textbf{Why it matters.} This file contains authentication UI, form controls, uploads, live capture trigger, session listing, WebSocket integration, derived data rows, and visual panels.

\textbf{Interview explanation.} This file demonstrates frontend state orchestration, data fetching, and result presentation in a single coherent page.
...
The project also has real limitations. PCAP and live inference remain heuristic-heavy. Evaluation metrics are strongest only for labeled data. The current frontend is implemented in one large page component and could be split into smaller modules. SQLite is sufficient for local use but not ideal for high-concurrency or distributed deployment. The current authentication model is adequate for a demo but not hardened for production. The legacy scripts suggest the repository contains exploratory artifacts that could be cleaned up further.
...
Possible backend improvements include token expiration, role-based access control, better upload validation, a persistent external queue, and migration from SQLite to PostgreSQL.

\section{Frontend Improvements}
The frontend could be refactored into smaller components, include richer charts, support filtering and sorting, and add export features for reports and CSV results.
...
I would improve protocol-specific flow modeling, split the frontend into smaller components, harden authentication, move to PostgreSQL for hosted deployments, and build a richer benchmarking suite for capture data.
...
This project demonstrates a controlled traffic-analysis platform where metadata from simulated, replayed, offline packet captures, and live network observation is transformed into correlation candidates, ranked routing paths, and probabilistic source-region estimates. The platform is implemented as a modular Python engine with a Node backend and a React-based dashboard.

\section{Resume Bullet 1}
Developed a Python-based traffic-correlation engine with feature extraction, hop scoring, path ranking, probabilistic region estimation, and evaluation across simulated, replayed, PCAP, and live-capture inputs.

\section{Resume Bullet 2}
Built a full-stack dashboard using Express.js, SQLite, WebSockets, Next.js, and Tailwind CSS for authenticated session management, dataset uploads, queued analysis jobs, and real-time result visualization.

\section{Resume Bullet 3}
Integrated TShark-based packet metadata ingestion and refined PCAP burst clustering logic to convert real packet captures into structured analysis sessions suitable for academic traffic-analysis demonstrations.

\chapter{Final Assessment}
This project is sufficiently broad and technically rich to discuss in an interview or internship setting. It demonstrates system design, cross-language integration, algorithmic modeling, asynchronous orchestration, UI engineering, testing, and documentation. Its strengths should be presented confidently, and its limitations should be presented honestly. That combination usually leaves a stronger impression than overclaiming.
...
\texttt{backend/db.js} & SQLite initialization and data access \\
...
\texttt{frontend/app/page.js} & Main dashboard page \\
