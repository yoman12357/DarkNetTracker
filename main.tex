...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item jobs.
\end{itemize}
This ensures that session history, authentication state, and administrative audit trails remain available even after multiple requests and page refreshes. The final implementation also enables SQLite WAL mode, foreign-key enforcement, and a busy timeout so the local database behaves more reliably under repeated dashboard activity. The schema was also prepared for future PostgreSQL migration by keeping the stored structures explicit and portable.
...
Each analysis request becomes a queued job. The job runner marks jobs as queued, running, completed, or failed, and the improved version also supports retry attempts and stale-job recovery so interrupted work does not remain permanently stuck in the running state. This is especially useful for live capture mode where analysis is not instantaneous.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item multi-seed simulation benchmark regression.
\end{itemize}

\section{Benchmarking}
To strengthen the evaluation story beyond single-case screenshots, the project now includes a repeatable Python benchmark command that runs the simulation pipeline across multiple random seeds and reports aggregate accuracy and top-region hit rate. This does not turn the project into a formal research benchmark suite, but it does provide a more defensible regression baseline for future tuning.
...
\texttt{tests/test\_benchmark\_regression.py} & Multi-seed simulation benchmark regression test \\
