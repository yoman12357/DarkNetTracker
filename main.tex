...
\end{enumerate}
This part was refined during development because real PCAP files initially produced over-compressed outputs.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item jobs.
\end{itemize}
This ensures that session history remains available even after multiple requests and page refreshes.

\section{Authentication}
A token-based authentication mechanism protects the API. A default administrative account is seeded for local demo use. The frontend performs login and then sends the issued token automatically with authenticated requests.
...
The backend uses authentication, local persistence, and server-side job handling. Uploaded files are processed locally. However, production hardening such as role-based access control, token expiration, secure secret management, and hardened deployment has not been the primary focus of this academic implementation.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item protocol-specific flow modeling,
    \item improved scoring for DNS and mixed-flow captures,
    \item exportable PDF/CSV reports from the dashboard,
    \item more detailed graph visualization.
\end{itemize}

\section{Medium-Term Improvements}
\begin{itemize}[leftmargin=1.5em]
    \item multi-user support,
    \item token expiration and user management,
    \item background worker scaling,
    \item Postgres-based persistence,
...
\texttt{backend/db.js} & SQLite persistence layer \\
\texttt{backend/queue.js} & Job queue logic \\
\texttt{frontend/app/page.js} & Web dashboard UI \\
\bottomrule
\end{longtable}
